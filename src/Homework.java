...
public class Homework {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        System.out.println("Negative Query: ");
        for (String negate : negatedQueryList) {
            System.out.println(negate);
        }
        System.out.println("KB sentences: ");
        ...
        System.out.println("This is unifyKB");
        while (iterator.hasNext()) {
            System.out.println(iterator.next());
        }
        // KB into predicate and Literal
        HashSet<Literal> standKB = standardizeKB(KB);
        System.out.println("This is standKB");
        Iterator standiterator = standKB.iterator();
        while (standiterator.hasNext()) {
            System.out.println(standiterator.next());
        }
        HashMap<Predicates, ArrayList<Literal>> indexKB = generateIndexKB(standKB);
        Iterator indexiterator = indexKB.entrySet().iterator();
        System.out.println("This is indexKB");
        ...
        Iterator prePorcessiterator = preProcessKB.entrySet().iterator();
        System.out.println("This is preProcessKB");
        while (prePorcessiterator.hasNext()) {
            System.out.println(prePorcessiterator.next());
        }
        output( preProcessKB,  indexKB,  negatedQueryList);
        // Test validConstant
//        Predicates currentPredicate = standardize(negatedQueryList[0]);
//        currentPredicate = currentPredicate.negativePredicate();
//        Predicates result = getIndexPredicate(preProcessKB, currentPredicate);
//        System.out.println(result.toString());
    ...
    public static Predicates getIndexPredicate(HashMap<PredicateIndex, ArrayList<Predicates>> preProcessKB, Predicates currentPredicate) {
        ...
        for (Predicates p : list) {
            if (currentPredicate.validComboConstant(p)) {
                result = p;
                return result;
            }
        }
...
//        ArrayList<Boolean> answerList = new ArrayList<>();
        for(Literal currentLiteral : nextTurn){
            ...
            System.out.println("RESOLUTION: " + currentLiteral.toString());
    ...
    public static void singleResolution(ArrayList<ResolutionResult> result, ArrayList<Literal> current, int index, HashMap<PredicateIndex, ArrayList<Predicates>> preProcessKB, HashSet<Literal> unsolved, Literal currentQuery, HashMap<Predicates, ArrayList<Literal>> indexKB){
        ...
        //当前层
           ...
           currentPredicate = currentPredicate.negativePredicate();
           Predicates getIndex = getIndexPredicate(preProcessKB, currentPredicate);
           // case1
           if (getIndex == null) {
               ...
               ArrayList<Literal> KBLiteral = indexKB.get(getIndex);
               if (KBLiteral == null || KBLiteral.size() == 0) {
                   Literal unsolvable = new Literal(new ArrayList<>(Arrays.asList(currentPredicate.negativePredicate())));
                   unsolved.add(unsolvable);
                   singleResolution(result, current, index + 1, preProcessKB, unsolved, currentQuery, indexKB);
                   unsolved.remove(unsolvable);
               } else if (KBLiteral.size() == 1) {
                   if (KBLiteral.get(0).validCombo(currentPredicate)) {
                       Literal afterResolution = KBLiteral.get(0).resolve(currentPredicate);
                       current.add(afterResolution);
                       singleResolution(result, current, index + 1, preProcessKB, unsolved, currentQuery, indexKB);
                       current.remove(afterResolution);
                   } else {
                       Literal unsolvable = new Literal(new ArrayList<>(Arrays.asList(currentPredicate)));
                       unsolved.add(unsolvable);
                       singleResolution(result, current, index + 1, preProcessKB, unsolved, currentQuery, indexKB);
                       unsolved.remove(unsolvable);
                   }
